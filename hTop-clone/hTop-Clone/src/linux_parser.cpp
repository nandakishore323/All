...
float LinuxParser::MemoryUtilization() { 
  ...
  auto avail_memory = static_cast<float>(LinuxParser::ReadProcMem("MemAvailable"));
...
string LinuxParser::Ram(int pid) { 
    const int pid_ = pid;
    const string key_ = "VmRSS";
    const auto mbMem = (LinuxParser::ReadProcPID(pid_, key_) / 1024);
